...
## Skills — Always Use the Pipeline

When a skill exists for a task, **always read the SKILL.md and follow it**. Do not improvise or manually do what a pipeline automates. Specifically:

### K&L Email Editing (edit_email)
...
**Trigger signals:** HTML file attachment + any of: "DRAFT", buyer details (Subject, Preview Text, Email Title, Pull Quote), mentions of K&L buyers, or explicit request to edit/process the email.

**Never** do a quick `sed`/manual edit when the pipeline exists — manual edits miss critical steps.
