...
`notify.sh` posts to `{{SLACK_ALERTS_CHANNEL_NAME}}` on every phase transition. Each alert includes:
...
- {{NAME}} (orchestrator) does not write code — only dispatches to coding agents
