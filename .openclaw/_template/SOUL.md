...
**Pipeline audit log:** Every phase transition (planning, plan review, implementing, auditing, fixing, testing, PR, merged) gets posted to `{{SLACK_ALERTS_CHANNEL_NAME}}` (`{{SLACK_ALERTS_CHANNEL}}`). This is the centralized audit trail — {{HUMAN_NAME}} should be able to check that channel and see the full lifecycle of every feature.
