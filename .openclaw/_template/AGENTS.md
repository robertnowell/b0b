...
| Me ({{NAME}}) | Agents (Claude Code / Codex) |
|---|---|
...
0.5. **Always ping `<@{{SLACK_REVIEW_USER}}>` when a fix/audit/test agent completes with ZERO new commits.** This means the agent likely misread feedback or silently failed. Don't just post the warning — tag {{HUMAN_NAME}} so he sees it immediately. Every change goes through a branch + PR, no matter how small. No exceptions. This is a failsafe that must never be violated.
...
6. **Always post the FULL plan to `{{SLACK_PROJECT_CHANNEL_NAME}}` ({{SLACK_PROJECT_CHANNEL}}) when a plan reaches plan_review.** Not a summary — the entire plan text. Then ping `<@{{SLACK_REVIEW_USER}}>` so {{HUMAN_NAME}} gets notified. This is the primary review surface. Do this every time, no exceptions.
7. **Always post to `{{SLACK_ALERTS_CHANNEL_NAME}}`** (`{{SLACK_ALERTS_CHANNEL}}`) for every phase transition — planning, plan review, implementing, auditing, fixing, testing, PR, merged. This is the audit log.
8. **Always post to `{{SLACK_ALERTS_CHANNEL_NAME}}` before spawning any agent** — pipeline agents, sub-agents, investigation agents, anything. The human should see "X started" before the work begins, not after.
...
# $REPO_ROOT is {{WORKSPACE_REPO_PATH}} (set in config.sh)
