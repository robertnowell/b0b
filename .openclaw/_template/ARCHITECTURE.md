# ARCHITECTURE.md — {{NAME}} Agent Orchestration System

> Last updated: 2026-02-28

## 1. System Overview

{{NAME}} is an AI engineering orchestrator that manages the full lifecycle of software development tasks for the **Kopi AI** product (trykopi.ai). It runs inside [OpenClaw](https://openclaw.com), a platform for persistent AI agents with tool access, cron jobs, and messaging integrations.
...
**Key principle:** {{NAME}} is an orchestrator, not a coder. Its value is product judgment, task decomposition, and quality gates.
...
│                 │                              │                     │
│     ┌───────────▼──────────┐      ┌────────────▼──────────────┐    │
│     │  Cron: monitor-      │      │  Cron: gh-poll-{{WORKSPACE_NAME}}   │    │
...
│  │  {{SLACK_ALERTS_CHANNEL_NAME}}   │    │  (notify-outbox.jsonl)        │      │
│  │  {{SLACK_PROJECT_CHANNEL_NAME}}  │    │  + Slack Webhook              │      │
│  └──────────────────────┘    └───────────────────────────────┘      │
└─────────────────────────────────────────────────────────────────────┘
        │
        │ tmux sessions
...
All scripts live in `$REPO_ROOT/.clawdbot/scripts/` (committed to the Rendition repo). `$REPO_ROOT` is `{{WORKSPACE_REPO_PATH}}`.

### config.sh
Shared configuration sourced by every other script. Defines paths:
- `REPO_ROOT` → `{{WORKSPACE_REPO_PATH}}` (git rev-parse)
- `CLAWDBOT_DIR` → `$REPO_ROOT/.clawdbot` (scripts + prompts, committed)
- `STATE_DIR` → `~/.openclaw/{{WORKSPACE_NAME}}/pipeline` (runtime state, not committed; override with `CLAWDBOT_STATE_DIR` env var)
...
- Appends to `notify-outbox.jsonl` for async relay by {{NAME}}
...
**GitHub mention poller.** Checks for `@{{BOT_USER}}` mentions in the product repo.
...
The central task registry. Lives at `~/.openclaw/{{WORKSPACE_NAME}}/pipeline/active-tasks.json` (runtime state, not committed to repo). Array of task objects, protected by flock on `.tasks.lock`.
...
```jsonc
{
  ...
  "worktree": "{{WORKTREE_BASE}}/ios-visibility-bug",
...
| `gh-poll-{{WORKSPACE_NAME}}` | Every 2 min | Isolated | Runs `gh-poll.sh` — checks for @{{BOT_USER}} GitHub mentions |

### launchd Agent

`com.openclaw.{{WORKSPACE_NAME}}.monitor` — runs `$REPO_ROOT/.clawdbot/scripts/monitor.sh` every 5 minutes as a macOS launch agent. This is a redundant execution path alongside the OpenClaw cron (belt and suspenders).
...
| Channel | ID | Purpose |
|---|---|---|
| `{{SLACK_ALERTS_CHANNEL_NAME}}` | `{{SLACK_ALERTS_CHANNEL}}` | **Pipeline audit log.** Every phase transition, every agent spawn, every verdict. The single source of truth for task status. |
| `{{SLACK_PROJECT_CHANNEL_NAME}}` | `{{SLACK_PROJECT_CHANNEL}}` | Discussion channel for the {{NAME}} project itself. |
...
{{NAME}} posts via `accountId={{WORKSPACE_NAME}}` (bound in OpenClaw config). The default account (`kl`) cannot access these channels.

## 8. Prompt System

### Templates

Standard templates live in `$REPO_ROOT/.clawdbot/prompts/` (committed to repo). Task-specific one-off prompts live in `~/.openclaw/{{WORKSPACE_NAME}}/pipeline/prompts/`. All templates use `{VAR_NAME}` placeholder syntax.
...
|---|---|---|---|
| Claude Code | `{{CLAUDE_PATH}}` | Frontend, UI, git ops, faster | `claude --model claude-opus-4-6 --dangerously-skip-permissions -p - < prompt` |
| Codex | `/Applications/Codex.app/Contents/Resources/codex` | Backend, complex bugs, reasoning | `codex exec --dangerously-bypass-approvals-and-sandbox < prompt` |
