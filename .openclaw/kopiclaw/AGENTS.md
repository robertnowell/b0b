...
6. **Always post the FULL plan to `#project-kopi-claw` (C0AJAR3S76U) when a plan reaches plan_review.** Not a summary — the entire plan text. Then ping `<@UXXXXXXXXXXXX>` so the reviewer gets notified. This is the primary review surface. Do this every time, no exceptions.
