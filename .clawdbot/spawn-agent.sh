...
if [ "$AGENT_TYPE" = "codex" ]; then
  /Applications/Codex.app/Contents/Resources/codex exec --dangerously-bypass-approvals-and-sandbox < "$AGENT_PROMPT_FILE" 2>&1 | tee -a "$AGENT_LOG_FILE"
elif [ "$AGENT_TYPE" = "claude" ]; then
  /Users/kopi/.local/bin/claude --model "$AGENT_MODEL" --dangerously-skip-permissions -p - < "$AGENT_PROMPT_FILE" 2>&1 | tee -a "$AGENT_LOG_FILE"
fi
