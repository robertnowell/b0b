...
# Reads .clawdbot/active-tasks.json and checks:
#   - tmux session alive?
...
MAX_RUNTIME_SECONDS="${MAX_RUNTIME_SECONDS:-2700}"  # 45 minutes default
...
try:
    ...
    for task in tasks:
      try:
        ...
        logfile = task.get('logFile', f'{repo_root}/.clawdbot/logs/agent-{tid}.log')
...
" "$REPO_ROOT" "$TASKS_FILE" "$LOCK_FILE" "$MAX_RUNTIME_SECONDS"
